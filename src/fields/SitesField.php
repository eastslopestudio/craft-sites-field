...
class SitesField extends Field implements PreviewableFieldInterface
{
    ...
    {
        $uids = $this->normalizeValue($value, $element);
        $uids = is_array($uids) ? array_filter($uids) : array_filter([$value]);
        ...
        if (count($uids) > 5) {
            return count($uids) . ' sites';
        }

        $siteNames = [];
        foreach ($uids as $uid) {
            if ($site = Craft::$app->getSites()->getSiteByUid($uid)) {
                $siteNames[] = $site->name;
            }
        }
